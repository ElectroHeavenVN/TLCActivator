...
namespace TLCActivator.LicenseCheckBypass
{
    static class Hook
    {
            ...
            static bool Prefix(Control __instance, ref string value)
            {
                ...
                if ((value.StartsWith("[thanhlc.com] [HSD: ") || value.StartsWith("[thanhlc.com] HSD: ") || value.StartsWith("\ud835\udcd3\ud835\udc93\ud835\udc82\ud835\udc88\ud835\udc90\ud835\udc8f \ud835\udcd1\ud835\udc82\ud835\udc8d\ud835\udc8d \ud835\udcdf\ud835\udc93\ud835\udc90 \ud835\udfd0.\ud835\udfd1.\ud835\udfd5")) && __instance is Form)
                {
                    ...
                    if (!isFormTitleSet && Main.IsProductIDUnknown)
                        MessageBox.Show(__instance, $"Product ID: {Main.newProductID}\r\nProduct type: {Main.productType}", "Information", MessageBoxButtons.OK, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1, (MessageBoxOptions)0x00040000);
                    isFormTitleSet = true;
                }
        ...
        public class StringConcatMultipleHook
        {
            ...
            {
                if (values.Contains(Main.cpuInfo) && values.Contains(Main.ramInfo) && values.Contains(Main.hwInfo) && Main.IsProductIDUnknown && values[0] != Main.productID)
                {
                    if (Main.newProductID != values[0])
        ...
        public class StringContainsHook
        {
            static bool Prefix(string __instance, string value, ref bool __result)
            {
                ...
                if (__instance == Main.productType && Main.IsProductTypeUnknown && Main.productType.IndexOf(value, StringComparison.Ordinal) < 0)
                {
